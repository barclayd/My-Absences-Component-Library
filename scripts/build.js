...
const shouldPublish = !!parseInt(process.env.PUBLISH_PACK, 10);
// check if a version tag is provided
const hasVersionTag = !!process.env.TAG;

// Define reusable constants
const COMPONENTS = 'src/stories';
...
const listFiles = () => {
  log('Finding files in .stories/ directory');
...
function createExtraFiles() {
  ...
  const filePromises = [].map((f) => copyFile(f));
...
function runNPMProcess(script, ...args) {
  const childProcess = spawn('npm', [script, ...args], {
    sdio: 'inherit',
    cwd: 'dist/',
  });
  return new Promise((resolve, reject) => {
...
// Additionally, it creates an index.js in file to allow importing
// the whole library (and allow retrocompatibility),
...
async function build() {
  try {
    ...
    // Generate README, index.css, package.json of dist dir
